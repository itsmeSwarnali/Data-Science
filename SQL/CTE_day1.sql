...
--Q1 — CTE Practice (Simpler Version)
--Using a WITH CTE, find customers whose total spent across all their orders (any status) is above €600.
--Show: full_name, country, total_amount.
with total_spent_table as (
  select c.full_name, c.country, sum(o.amount) as total_amount
  from customers c inner join orders o on c.customer_id = o.customer_id
  group by c.full_name, c.country
  )
select * from total_spent_table where total_amount>600;


--Q2 — CTE + Comparison To Average (Same Shape As Your Example!)
--Using a WITH CTE, find products whose total revenue (quantity × unit_price, summed) is above the average total revenue across all products.
--Show: product_name, category, total_revenue.
with total_revenue_table as (select p.product_name, p.category, 
sum(oi.quantity * oi.unit_price) as total_revenue
from products p inner join order_items oi on p.product_id = oi.product_id
group by p.product_name, p.category)
select * from total_revenue_table where total_revenue > (select avg(total_revenue) from total_revenue_table);

--Decide First: GROUP BY or Window?


--"Show each customer's order_id, order_date, and amount, 
--along with a column showing how many total completed orders that customer has made.
--" — does this need GROUP BY or a Window Function? Why? each row per customer od order_id, date and amount, so window func. 
--in question "Show each customer's " for that we have to do partition by customer_id
select o.customer_id, o.order_id, o.order_date, o.amount,
count(o.status) over (partition by o.customer_id) as total_status
from orders o
