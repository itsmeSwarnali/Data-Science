...
--Q1 — CTE Practice (your strength now!)
--Using a WITH CTE, find countries whose total number of completed orders is above the average number of completed orders per country.
--Show: country, completed_order_count.
with completed_order_table as (select
c.country, sum(case when o.status = "completed" then 1 else 0 end) as completed_order_count
