...
-- Q1 — Show each order with the customer full name, order amount, and product name that was ordered.
--(Hint: You need to join 3 tables: customers → orders → order_items → products)
select o.order_id, c.full_name, o.amount, p.product_name
from customers c
inner join orders o 
...
-- Q2 — Show all products and how many times they have been ordered.
--Show: product name, category, total quantity sold.
...
-- Q3 - For each product category, what is the total revenue generated? Revenue = quantity × unit_price
-- Show: category, total revenue.
...
-- Q4 -- CASE WHEN Introduction 🆕 Categorize each customer by their total_spent into segments:
--'VIP' → total_spent > 800
--'Regular' → total_spent between 400 and 800
--'Low Value' → total_spent < 400
select c.full_name, c.country, c.total_spent,
  case
    when c.total_spent > 800 then 'VIP'
    when c.total_spent >= 400 and c.total_spent <= 800 then 'Regular'
    when c.total_spent < 400 then 'Low Value'
  end as segment
from customers c;
...
-- Q5 -- JOIN + GROUP BY + CASE WHEN For each country, show:
--Total number of customers
--Total revenue from completed orders
--Country segment: 'High Revenue' if total > 1000, else 'Low Revenue'
select c.customer_id, c.country, sum(oi.quantity * oi.unit_price) as total_revenue,
case
  when sum(oi.quantity * oi.unit_price)>1000 then 'High Revenue'
  else 'low revenue'
end as segment
from order_items oi
inner join orders o
