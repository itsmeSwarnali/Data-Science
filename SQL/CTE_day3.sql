...
--Q1 — CTE: Filter By Fixed Value. Using a WITH CTE, find categories whose total quantity sold is above 5 units.
--Show: category, total_quantity.
...
select * from quantity_table where total_quantity>5;

--Q2 — CTE: Compare To AVG (same shape as your product-revenue question)
--Using a WITH CTE, find countries whose average customer total_spent is above the average total_spent across 
--ALL customers company-wide.
--Show: country, country_avg_spent.
with average_customer_total_spent_table as
(

  select c.country, avg(c.total_spent) as country_avg_spent
...
select * from average_customer_total_spent_table where country_avg_spent > (select avg(country_avg_spent) from average_customer_total_spent_table);



--Q3: Find countries whose average customer spending is above the company-wide average customer spending.
with average_customer_total_spent_table as
(
...
select * from average_customer_total_spent_table where country_avg_spent > (select avg(c.total_spent) from customers c);



--Q4 — CTE: Two-Level Aggregation (slightly harder rep)
--Using a WITH CTE, find customers whose number of distinct product categories purchased is above 1 
--(meaning they bought from more than one category).
--Show: full_name, country, distinct_category_count.
with distinct_product_categories_table as 
(

  select c.full_name, c.country, count(distinct p.category) as distinct_category_count
...
select * from distinct_product_categories_table where distinct_category_count>1;
