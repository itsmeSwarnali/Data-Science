...
-- Q1 -- Build a simple customer feature table — one row per customer — with: full_name, country
-- total number of orders placed (any status)
-- total number of completed orders
-- total amount spent on completed orders (0 if none)
-- This is exactly the kind of table you'd export to a CSV and load into pandas for a churn model.
select c.full_name, c.country, count(o.order_id) as total_number_of_orders_placed, count(case when o.status = "completed" then 1 end) as total_number_of_completed_orders, 
sum(case when o.status = "completed" then amount else 0 end) as total_amount_spent_on_completed_orders
from customers c left join orders o on c.customer_id = o.customer_id
group by c.full_name, c.country;

--Q2 --Window Function (a real DS pattern: recency)
--For each customer's completed orders, show order_id, customer_id, order_date, amount, and 
--a column showing how many days passed since their PREVIOUS completed order (0 or NULL for their first order).
--(This is the basis of "days since last purchase" — a classic churn feature!)
